...
import { MessageSquare, Send, ArrowLeft, MoreVertical, Phone, Video, Search, Check, CheckCheck, Clock, Paperclip, Image as ImageIcon } from 'lucide-react';
...
export default function CustomerInboxPage() {
  ...
  const openConversation = async (conversation: Conversation) => {
    try {
      const response = await fetch(`/api/customer/conversation?vendorId=${conversation._id}`);
  ...
  const sendFile = async (file: File, messageType: 'image' | 'document') => {
    ...
    try {
      ...
      const messageResponse = await fetch('/api/customer/conversation', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const getVendorColor = (vendorId: string) => {
    ...
    const index = vendorId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    ...
    return (
      <div className="h-screen flex flex-col bg-gray-100">
        ...
        <div className="bg-green-500 text-white px-4 py-3 flex items-center gap-3 shadow-lg">
          ...
          <div className={`w-10 h-10 ${getVendorColor(selectedConversation.vendor._id)} rounded-full flex items-center justify-center text-white font-semibold text-sm`}>
            {getVendorInitials(selectedConversation.vendor.name)}
          </div>
          
          <div className="flex-1 min-w-0">
            <h2 className="font-semibold text-lg truncate">{selectedConversation.vendor.name}</h2>
            <p className="text-green-100 text-sm truncate">{selectedConversation.vendor.serviceType}</p>
          </div>
  ...
  return (
    <div className="h-screen flex flex-col bg-white">
        ...
        {conversations.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {conversations
              ...
              >
                <div className={`w-12 h-12 ${getVendorColor(conversation._id)} rounded-full flex items-center justify-center text-white font-semibold text-sm flex-shrink-0`}>
                  {getVendorInitials(conversation.vendorName)}
                </div>
      ...
      <CallModal
        ...
        callType={callType}
        participantName={selectedConversation?.vendor.name || 'Vendor'}
        participantServiceType={selectedConversation?.vendor.serviceType || 'Service'}
